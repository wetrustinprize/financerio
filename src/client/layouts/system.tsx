...
function Wallets() {
  ...
  const WalletButton = ({
    ...
    return (
      <Button
        ...
        variant={isHere ? 'ghost' : 'light'}
        onPress={() => navigate(`/wallets/${id}`)}
      >
        <p>{name}</p>
        <p>{total.toFixed(2)}</p>
      </Button>
  ...
  return (
    <SidebarSection icon={MdOutlineAccountBalanceWallet} label="Wallets">
      ...
      {wallets.map((wallet) => (
        <WalletButton id={wallet.id} name={wallet.name} total={0} />
      ))}
