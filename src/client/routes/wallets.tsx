...
export default function Wallets() {
  ...
  const bulkDelete = () => {
    ...
    if (selection instanceof Set)
      z.mutateBatch((tx) =>
        selection.forEach((id) => tx.wallets.delete({ id: id as string })),
      );
    else z.mutate.categories.delete({ id: '*' });
    ...
    <section className="flex flex-col gap-2 flex-1">
      <section className="p-2 flex justify-between">
        <h1 className="font-bold text-4xl">
          {wallet.length > 0 ? wallet[0].name : 'All wallets'}
        </h1>
        <Input
          isClearable
          onClear={() => setSearch('')}
      ...
      <section className="flex">
        <section className="flex-1 flex">
          <Button color="primary" onPress={() => console.log('TODO')}>
            Create transaction
          </Button>
        </section>
        <section className="flex-1 flex flex-row-reverse">
          <Button
            color="danger"
            onPress={bulkDelete}
            isDisabled={selection instanceof Set && selection.size === 0}
            isIconOnly
          >
            <MdDeleteOutline />
          </Button>
      ...
      <Table
        ...
        classNames={{ base: 'flex-1', wrapper: 'flex-1' }}
      >
        <TableHeader>
        ...
        <TableBody emptyContent={'No transactions'}>
          {transactions.map((transaction) => (
            <TableRow key={transaction.id}>
              ...
              <TableCell>{transaction.relatedWallet?.name}</TableCell>
              <TableCell>{transaction.relatedCategory?.name}</TableCell>
